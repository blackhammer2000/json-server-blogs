  ...
  try {
    ...
    for (const property in newData) {
      if (
        ...
        property.toLowerCase().includes("reactions")
      ) {
...
router.patch("/api/reactions/edit/comment", async (req, res) => {
  try {
    ...
    res.status(203).json({ message: "comment updated", updatedComments });
  } catch (err) {
    if (err.message) res.status(500).json({ error: err.message });
  }
});
...
router.patch("/api/reactions/comment/update/reply", async (req, res) => {
  try {
    ...
    res
      .status(203)
      .json({ message: "comment reply updated", comments: updatedComments });
  } catch (err) {
    if (err.message) res.status(500).json({ error: err.message });
  }
});
...
router.patch("/api/reactions/comment/reply/like", async (req, res) => {
  try {
    ...
    res.status(203).json({
      message: `comment ${liked ? "unliked" : "liked"}`,
      replyLikes: newLikes,
    });
  } catch (err) {
    if (err.message) res.status(500).json({ error: err.message });
